...
Le System calls è `open, write, close`.
...
per rendere più facile l’uso e migliorare la portabilità delle
applicazioni.

Le API agevolano la vita noi sviluppatori 🧑‍💻, nel percorso di SO-lab useremo delle API su `C` (ma sono _molto_ simili).

## Gestione dei processi

In un dato istante sono attivi più processi in un momento, un solo è programma è in esecuzione. Si parla di **processi** concorrenti perchè questi competono per usare le risorse hardware delle macchina:
...
Quasi ogni informazione presente nel sistema è contenuta in un **file**, essi sono organizzati in una gerarchia detta **File System** mediante **cartelle** (o directory, o folder).
